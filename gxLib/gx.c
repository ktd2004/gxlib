...
변경일자 :  2009-10-10 24bpp 칼라순서를 RGB --> BGR 로 변경, BPP 값을 4로 변경
...
dc_t    *gx_get_compatible_dc( dc_t *dc_sour)                                   // 인수 dc에 호환되는 dc_t를 구함
{
   ...
   if ( NULL != dc)                                                             // 메모리 구하기에 성공함녀
   {
      ...
      dc->release_dc        = dc_sour->release_dc;
...
int gx_open_file( dc_t *dc, char *filename)
{
    ...
    if ( 0 == strcasecmp( file_ext, "bmp"))
    {
        ...
        if ( NULL == bmp)
            gx_print_error( filename);                                              // 실행 중 에러 내용을 출력
        else
        {
...
// 반환: 버퍼 Device Context 핸들
{
   ...
   if ( NULL != dc)                                                             // 메모리 구하기에 성공함녀
   {
      ...
      case 24  :  dc->bytes_per_line  = width * 4; break;
...
{                                                                               
   char *error_string[] ={ "에러 없음",                                         // GXERR_NONE
                           ...
                           "이미지 처리를 위한 DC 생성에 실패",                 // GXERR_NO_CANVAS
                           "파일 이름 오류"                                     // GXERR_FILE_NAME
                        };
